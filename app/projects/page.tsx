...
export default function ProjectsPage() {
  ...
  const [purgingId, setPurgingId] = useState<string>("");
  ...
  const loadTrash = async () => {
    ...
    try {
      ...
      const rows = ((data?.projects || []) as ProjectRow[]).slice();
      setTrashed(rows);
      if (Number.isFinite(data?.retentionDays)) setRetentionDays(data.retentionDays);
    } catch (e: any) {
      const msg = String(e?.message || e || "").toLowerCase();
      if (msg.includes("auth") || msg.includes("session") || msg.includes("jwt")) {
        redirectToLogin();
      ...
      setTrashError(e?.message || "Failed to load recycle bin");
    } finally {
  ...
  // Days left before a binned project is permanently purged.
  const daysLeft = (project: ProjectRow): number => {
    if (!project.deleted_at) return retentionDays;
    const deletedMs = new Date(project.deleted_at).getTime();
    if (!Number.isFinite(deletedMs)) return retentionDays;
    const expiresMs = deletedMs + retentionDays * 24 * 60 * 60 * 1000;
  ...
  const deleteForever = async (project: ProjectRow) => {
    ...
    try {
      ...
      const res = await fetch(
        `/api/projects/${encodeURIComponent(project.id)}?permanent=true`,
        {
          method: "DELETE",
          credentials: "include",
          headers: authHeaders(),
  ...
  return (
    <div style={styles.page}>
      ...
      {binOpen && (
        <div style={styles.modalOverlay} onClick={() => setBinOpen(false)}>
          <div
            style={{ ...styles.modal, width: "min(720px, 96vw)", maxHeight: "85vh", overflowY: "auto" }}
          ...
          >
            ...
            >
              <div style={{ fontSize: 18, fontWeight: 800 }}>🗑 Recycle Bin</div>
              <button style={styles.clearBtn} onClick={() => setBinOpen(false)}>
                Close
              </button>
            </div>

            <div style={{ ...styles.subtitle, marginBottom: 14 }}>
              Deleted projects are kept for {retentionDays} days, then permanently removed.
            </div>

            {trashLoading ? (
              <div style={styles.stateBox}>Loading recycle bin...</div>
            ) : trashError ? (
              <div style={styles.stateBox}>{trashError}</div>
            ) : trashed.length === 0 ? (
              <div style={styles.stateBox}>Recycle bin is empty</div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                {trashed.map((p) => {
                  const left = daysLeft(p);
                  const deletedDt = p.deleted_at
                    ? new Date(p.deleted_at).toLocaleString()
                    : "—";
                  const isRestoring = restoringId === p.id;
                  const isPurging = purgingId === p.id;
                  ...
                  return (
                    ...
                    >
                      <div style={{ minWidth: 0 }}>
                        ...
                        <div
                          style={{
                            fontSize: 12,
                            fontWeight: 600,
                            color: left <= 3 ? "#dc2626" : "#0f766e",
                          }}
                        >
                          {left > 0
                            ? `${left} day${left === 1 ? "" : "s"} left before permanent deletion`
                      ...
                      <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
                        <button
                          type="button"
                          style={{ ...styles.btnGhostSmall, opacity: busy ? 0.7 : 1 }}
                          disabled={busy}
                        ...
                        <button
                          type="button"
                          style={{ ...styles.btnDangerSmall, opacity: busy ? 0.7 : 1 }}
                          disabled={busy}
